...
Depends on: TASK-003
Notes: see DEC-007, DEC-008, DEC-011, DEC-013, and DEC-014. Resolve the locked positive prompt with SUBJECT = `Sunwheat, a magical wheat analogue with plump golden seed heads, a faint warm inner glow, healthy green leaves, and one subtly dulled dusk-tinted seed head`; STATE OR VARIANT = `mature harvest-ready crop cluster`; MASTER WIDTH = `512`; MASTER HEIGHT = `512`. Use the negative prompt unchanged. The shadow must be separated from the approved crop source, not generated as a different plant silhouette. Rework required after PM review of commit `57c9237`: replace the crop candidate because it reads as a front-facing bouquet. The new crop must visibly look down onto the seed heads and leaves, use foreshortened stalks, and read as emerging from a ground plane rather than arranged upright for display. Regenerate the matching shadow from the replacement crop and update the metadata; do not retain the rejected crop silhouette as the reference.
