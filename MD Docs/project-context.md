...
- World perspective: use the same broad ground-versus-upright perspective convention as the supplied Stardew Valley farm screenshot, without copying its assets, pixel-art style, compositions, or interface. Flat terrain and soil use a straight screen-aligned top-down orthogonal square grid. Crops, props, characters, and buildings use high-angle front-elevated farm-game sprites: they rise upward on screen from a bottom-center ground contact, show a clear readable front plane plus modest visible top surfaces, and stack near elements lower/larger with rear elements higher/smaller. Upright subjects must not read as straight-down radial rosettes. Do not use an isometric projection, diamond tiles, 30-degree isometric axes, or diagonal grid movement.
